...
message = ""
$.shareuuid = "c0f1a4fdd379449f9e8f71bf5e124888"
    !(async () => {
        if (!cookiesArr[0]) {
        ...
        if (message.length != 0 && new Date().getHours() === 8) {
        if ($.isNode()) {
        ...
        $.post(taskPostUrl('/dingzhi/qqxing/pasture/activityContent', body), async (err, resp, data) => {
            ...
            try {
                if (err) {
                    ...
                    data = JSON.parse(data);
					//console.log(data) //uid的DATA信息
                        if (data.result) {                           
                           if(data.data.openCardStatus !=3){
                           console.log("21.0复制整段话 http:/J4XKytUE2DwuZh星系牧场养牛牛，可获得DHA专属奶！%F7Uzb5m6ia!咑開京岽app");
                           }
						   $.uid = data.data.uid
                           console.log(`\n【京东账号${$.index}（${$.UserName}）的${$.name}好友互助码】${$.uid}\n`);
                        }
                      }
            } catch (e) {
                $.logErr(e, resp)
