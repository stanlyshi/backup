...
2、如果需要使用临时禁止某些 COOKIE 参加所有活动或某些活动功能，以及全部随机或部分随机 COOKIE 参加活动的功能。还需要将 config_sample.sh 相应的环境变量设置模板复制到你的 /ql/config/config.sh 中，并根据要求填写；
3、如果需要使用组队利益最大化模板，需要将 team.sh 复制到青龙容器的 /ql/config/ 路径。用法详见 team.sh 说明。

### 〇、前言

code.sh 是基于青龙 2.2.0 的 code.sh 修改而来，仅支持 2.8.0 及以上版本的青龙，不支持 2.2.0及以下版本的青龙。主要功能包括：①批量格式化导出互助码和互助规则；②支持自定义互助规则模板；③支持指定活动采用指定互助模板；④支持临时屏蔽某账号被助力(这些账号仍可助力其他账号)；⑤支持自动静默安装或静默修复常用的 node 依赖包(支持自定义所需要安装的依赖包)；⑥支持 COOKIES 批量格式化导出；
task_before.sh 是本人基于青龙原版修改而来，支持包括东东农场、京喜工厂、摇钱树、京喜Token等16个活动的互助码、互助规则的格式化导出和提交。此外，配合置顶 config.sh 文件，可以实现临时禁止某些 COOKIE 参加所有活动或某些活动功能，以及全部随机或部分随机 COOKIE 参加活动的功能。
...
2、第二阶段：读取 code.sh 的日志和 /ql/log/.bak_ShareCode/ 路径下的各个活动的互助码和互助规则日志文件。逐条校对互助码和互助规则内容，如果不一致，则自动改写更新日志文件。当 code.sh 日志中互助码为 undefined 或 {} 时，跳过更新改写互助码和互助规则日志文件内容。

b) 有些情况下活动脚本输出的互助码不全，例如：你有 10 个账号，目前活动脚本只跑了一半，日志只输出了 5 个，那么 code.sh 整理的也就只有 5 个有互助码，5 个空值。那么互助码和互助规则日志文件的对应内容也会相应更新。
...
2、导出京喜农场 Token 的方法
重要提示：由于 Aaron-lv 等大佬的新版财富岛脚本内直接把 token 传递给提现接口实现提现，脚本代码已经不支持导出 TokenJxnc 变量，因此不用刻意追求导出)：
操作步骤：运行一次 code.sh，再根据日志提示的优先仓库，运行一次京喜财富岛活动脚本。再运行一次 code.sh 。
